...
class NoteSlotBar @JvmOverloads constructor(
    ...
    private fun rebuildSlots() {
        ...
        for ((index, note) in slotNotes.withIndex()) {
            container.addView(createSlotView(note, index))
            if (index < slotNotes.size - 1) container.addView(View(context).apply { layoutParams = LinearLayout.LayoutParams(dp(6), 0) })
        }

        for (i in slotNotes.size until slotCount) {
            container.addView(createEmptySlot(i))
            if (i < slotCount - 1) container.addView(View(context).apply { layoutParams = LinearLayout.LayoutParams(dp(6), 0) })
        }
    }
    ...
    private fun createSlotView(note: Note, index: Int): View {
        val textView = TextView(context).apply {
            ...
            val bgColor = if (note.id == currentNoteId) appColor else if (note.slotColor != 0) note.slotColor else 0xFF333333.toInt()
        ...
        // Long click strictly for Drag and Drop
    ...
    private fun startDrag(note: Note, index: Int, view: View) {
        ...
        view.startDragAndDrop(data, DragShadowBuilder(view), index, 0)
    }
    ...
    private fun setupDragListener() {
        container.setOnDragListener { _, event ->
            if (event.action == DragEvent.ACTION_DROP) {
                ...
                if (fromIndex != toIndex && toIndex >= 0) onSlotReorder?.invoke(fromIndex, toIndex)
            }
            true
        }
    }
    ...
    private fun getSlotIndexAtPosition(x: Float): Int {
        for (i in 0 until container.childCount) {
            ...
            if (child is TextView && x >= child.left && x <= child.right) return child.tag as? Int ?: -1
        }
        return -1
    }
